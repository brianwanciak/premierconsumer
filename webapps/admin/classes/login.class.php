...
class Login {
    ...
    private     $user_name                  = ""; 
	private     $user_group                  = "";                      // user's name
    ...
    private function loginWithPostData() {
            ...
            $checklogin = $this->connection->query("SELECT * FROM user WHERE email = '".$this->user_name."';");
            ...
            if($checklogin->num_rows == 1) {
                ...
                if (md5($_POST['user_password']) == $result_row->password) {
					...
					$_SESSION['user_username'] = $result_row->username;
					$_SESSION['user_fname'] = $result_row->fname;
                    $_SESSION['user_lname'] = $result_row->lname;
					$_SESSION['user_group'] = $result_row->group;
                    $_SESSION['user_logged_in'] = 1;
                    
                    /**
                     *  write user data into COOKIE [a file in user's browser]
                     */
                    setcookie("user_name", $result_row->username, time() + (3600*24*100));
